...
import stylesCss from 'themes/lumo/styles.css';
import '@vaadin/vaadin-lumo-styles/color.js';
import '@vaadin/vaadin-lumo-styles/typography.js';

window.Vaadin = window.Vaadin || {};
window.Vaadin['_vaadintheme_lumo_globalCss'] = window.Vaadin['_vaadintheme_lumo_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_lumo_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_lumo_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_lumo_globalCss'].push(target);
  }
